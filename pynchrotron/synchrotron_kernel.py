...
@nb.njit(fastmath=True, parallel=False)
def integrate_synchrotron(synchrotron_matrix, gamma, fgamma, n_grid_points, i):
...
def synchrotron_spectrum(
    ...
    new_emission = np.zeros(n_photon_energies)

    flag = True

    factor = np.zeros(n_grid_points)
    ...
    for i in range(n_photon_energies):
        ...
        for j in nb.prange(1, n_grid_points):
            ...
            if flag:
                ...
                factor[j] = fgamma[j] * (gamma[j] - gamma[j - 1])

                flag = False

            summ += synchrotron_matrix[i, j] * factor[j]

        # summ = integrate_synchrotron(synchrotron_matrix, gamma, fgamma, n_grid_points, i)

        new_emission[i] = emission[i] + summ / (2.0 * energy[i])

    return new_emission


__all__ = ["synchrotron_kernel", "compute_synchtron_matrix", "synchrotron_spectrum"]
